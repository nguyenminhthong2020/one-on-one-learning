...
import {searchCourseAsync} from '../../../../../redux/slices/course/searchCourseSlice';

import {ConvertLevel2} from '../../../../../redux/slices/course/searchCourseSlice';
import {_listCate} from '../../../../../redux/slices/course/searchCourseSlice';
...
const arrLevel = [
  ...
  const arrCategory = [
    // {item: 'For studying abroad', id: 0},
    // {item: "English for Traveling", id: 1},
  ...
  <View
    ...
    <View style={{padding: 8}}>
      ...
      <Text style={{textAlign: 'right', color: 'black'}}>
        {ConvertLevel(props.item.level)}     {props.item.topics.length} lessons
      </Text>
    </View>
  </View>
);
  ...
  return (
    <View style={{marginBottom: 0}}>
      <FlatList
        ...
        data={props.dataCourses}
        extraData={props.dataCourses}
...
const ListCourseNew = props => {
  ...
  useEffect(() => {
    const str = `course?page=${1}&size=100`;
    dispatch(
      searchCourseAsync({
        str: str,
  ...
  const _dataCourses = useSelector(state => state.searchcourse);
  const countPage = ~~(_dataCourses.data.count / 10) + 1;
  let arrCount = [];
  ...
  const onSearch = () => {
    let str = `course?page=${currentPage}&size=10`;
    if (query != '') {
      str = str + `&q=${query}`;
    }
  ...
  return (
    <>
      ...
      <View style={{paddingBottom: 310}}>
            <SecondRoute
              dataCourses={_dataCourses.data.rows}
              navigation={
                props.navigation
              } 
            />
              ...
              {arrCount.map((item, index) =>
                index + 1 == currentPage ? (
                  ...
                  <Pressable
                    key={index}
                    onPress={() => {
                      let str = `course?page=${index + 1}&size=10`;
                      if (query != '') {
                        str = str + `&q=${query}`;
                      }
                      if (arrLevelSelected.length > 0) {
